...
pveScene::pveScene()
{
    ...
    peopleFirstBtn->setParent(this);
    peopleFirstBtn->move(500,550);
    peopleFirstBtn->show();

    computerFirstBtn->setParent(this);
    computerFirstBtn->move(500,320);
    computerFirstBtn->show();
    ...
    //链接按钮
    connect(peopleFirstBtn,&QPushButton::clicked,[=](){
        peopleFirstBtn->zoomDown();
        peopleFirstBtn->zoomUp();
        QTimer::singleShot(400,this,[=](){
            peopleFirstBtn->hide();
            computerFirstBtn->hide();
    ...
    connect(computerFirstBtn,&QPushButton::clicked,[=](){
        computerFirstBtn->zoomDown();
        computerFirstBtn->zoomUp();
        QTimer::singleShot(400,this,[=](){
            peopleFirstBtn->hide();
            computerFirstBtn->hide();
...
//ai下棋的权重判断
int pveScene::value(int i, int j){
...
int pveScene::chessType(int rotate, int i, int j)
{
    if(aiFlag==1){
        int t,count=0;
...
void pveScene::moveByOne(int rotate, int *i, int *j){
    switch (rotate){
    case 1:{
        ++*i;
    }
...
//重写鼠标释放函数
void pveScene::mouseReleaseEvent(QMouseEvent *event){
    //判定是否开始以及鼠标位置是否正确
    if(!beginFlag) return;
